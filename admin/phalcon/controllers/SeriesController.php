...
class SeriesController extends BaseController
{
	...
	public function insertAction($viewType='')
	{
		if ($this->request->isPost()) {
			...
			var_dump($info);
			if( Series::count(" video_id = '{$info['video_id']}' ") > 0){
				exit;
			}
	...
	public function viewsAction($viewType='')
	{
		...
		$ProductCategory = ProductCategory::find(array('parent_category = 33 AND onshelf = 1 ',"order" => "sort ASC"));
	...
	public function onshelfAction()
	{
		if ($this->request->isPost()) {
			...
			$content = Series::findFirst(" id = $id ");
