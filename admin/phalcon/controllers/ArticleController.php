...
class ArticleController extends BaseController
{
	...
	public function insertAction($viewType='')
	{
		if ($this->request->isPost()) {
			...
			var_dump($info);
			if( Article::count(" video_id = '{$info['video_id']}' ") > 0){
				exit;
			}
	...
	public function viewsAction($viewType='')
	{
		...
		$ProductCategory = ProductCategory::find('parent_category = 2 AND onshelf = 1 ');
		$result = Article::find(array( "order" => "id DESC","limit" => 10));
		$result->rewind();
	...
	if ($this->request->isPost()) {
			...
			if(!isset($info['id']) || !$info['id']){
				$content  = new $info['data']();
			
				if ($content->create($perview) == false) {
					foreach ($content->getMessages() as $message) {
						echo $message, "\n";
					}
				}
	...
	public function onshelfAction()
	{
		if ($this->request->isPost()) {
			...
			$content = Article::findFirst(" id = $id ");
