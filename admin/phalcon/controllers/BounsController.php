...
class BounsController extends BaseController
{
	...
	public function insertAction($viewType='')
	{
		if ($this->request->isPost()) {
			...
			var_dump($info);
			if( Bouns::count(" video_id = '{$info['video_id']}' ") > 0){
				exit;
			}
	...
	public function viewsAction($viewType='')
	{
		...
		$ProductCategory = ProductCategory::find(array('parent_category = 34 AND onshelf = 1 ',"order" => "sort ASC"));
		
		$where = array( ' onshelf <= 1 ', "order" => "update_time ASC");
		
		
		
		$result = Bouns::find($where);
		$result->rewind();
